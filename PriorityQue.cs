...
namespace Assignment_2COIS2020H
{
    ...
    public class PriorityQueue<T> : IPriorityQueue<T> where T : IComparable
    {
        ...
        //Remove the Departure Event anywhere from the Priority queue (For interupts) 
        public Event RemoveEvent(int index)
        {
            if (!Empty())  //If priority queue isn't empty
            {
                Event removeEvent;
                for (int i = 1; i <= count; i++)  //check the Priority queue for the event
                {
                    removeEvent = A[i] as Event;

                    //Check if the event is in the right processor and is a departure
                    if (removeEvent.indexer == index && removeEvent.flag == false)
                    {
                        A[i] = A[count--]; //Replace the event with the last one in the Priority queue
                        PercolateDown(i);
                        return removeEvent;
                    }
                }
